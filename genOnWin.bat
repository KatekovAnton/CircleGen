...
mkdir build
cd build
call cmake -G "Visual Studio 16 2019" -T host=x64 -A x64 -DCMAKE_BUILD_TYPE=Debug -DCMAKE_INSTALL_PREFIX=%cd%/install/ .. 
...
mkdir build
cd build
call cmake -G "Visual Studio 16 2019" -T host=x64 -A x64 -DCMAKE_BUILD_TYPE=Debug  -DCMAKE_INSTALL_PREFIX=%cd%/install/ -DZLIB_LIBRARY=%masterfolder%/zlib/build/install/lib -DZLIB_INCLUDE_DIR=%masterfolder%/zlib/build/install/include .. 
